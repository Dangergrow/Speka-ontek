...
class Installer:
    ...
    def setup_ui(self):
        ...
        # Кнопка Готово (скрыта до конца установки)
        self.done_btn = Button(body, text="Готово", command=self.finish, font=('Segoe UI', 13, 'bold'), bg='#10b981', fg='white', bd=0, padx=40, pady=10, cursor='hand2')
        self.launch_btn = Button(body, text="🚀 Запустить ONTEK", command=self.launch_program, font=('Segoe UI', 13, 'bold'), bg='#1e40af', fg='white', bd=0, padx=40, pady=10, cursor='hand2')
    ...
    def install_done(self, shortcut_ok=False):
        ...
        if self.run_after.get() and self.exe_path and os.path.exists(self.exe_path):
            self.launch_btn.pack(in_=btn_frame, side='left', padx=5)
        
        self.done_btn.pack(in_=btn_frame, side='left', padx=5)
