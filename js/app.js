...
let idC=0,actId=null,ctxD={t:null,r:null,c:null},hist=[],uiMode='v2',theme='light',colorTheme='blue',recordingKey=null,recordingHk=null;
...
const keyLabels={addRow:'Добавить строку',delRow:'Удалить строку',addCol:'Добавить колонку',delCol:'Удалить колонку',recalc:'Пересчитать',paste:'Вставить',load:'Загрузить',newRUB:'Новая RUB',newUSD:'Новая USD',dup:'Дублировать',clear:'Очистить'};
...
// Сохранение
function saveAll(){try{const s={ui:uiMode,theme:theme,color:colorTheme,hotkeys:hotkeys};localStorage.setItem('ontek_settings',JSON.stringify(s));}catch(e){}}
function loadAll(){try{const s=JSON.parse(localStorage.getItem('ontek_settings'));if(s){uiMode=s.ui||'v2';theme=s.theme||'light';colorTheme=s.color||'blue';if(s.hotkeys)hotkeys=s.hotkeys;}}catch(e){}}
function updateAllHKDisplays(){
    document.querySelectorAll('.hk[data-hk]').forEach(el=>{const k=el.dataset.hk;if(hotkeys[k])el.textContent='Shift+'+hkDisplay(hotkeys[k]);});
...
function applyAllSettings(){
    ...
    Q('#uiV1').classList.toggle('active',uiMode==='v1');Q('#uiV2').classList.toggle('active',uiMode==='v2');
    Q('#toolbarV1').style.display=uiMode==='v1'?'block':'none';Q('#tablesAreaV1').style.display=uiMode==='v1'?'block':'none';
...
function switchUI(mode){uiMode=mode;applyAllSettings();if(mode==='v1'){Q('#tablesAreaV1').style.display='block';Q('#layoutV2').classList.remove('active');}else{Q('#tablesAreaV1').style.display='none';Q('#layoutV2').classList.add('active');reRenderAll();}}
...
function buildToolbarV1(){
    ...
    const s=[{lbl:'Строки',b:[{id:'btnAddRow1',c:'b-blue',t:'＋',h:'addRow'},{id:'btnDelRow1',c:'b-red',t:'－',h:'delRow'}]},{lbl:'Колонки',b:[{id:'btnAddCol1',c:'b-green',t:'＋',h:'addCol'},{id:'btnDelCol1',c:'b-orange',t:'－',h:'delCol'}]},{lbl:'Поиск',b:[{id:'btnFind1',c:'b-indigo',t:'🔍',h:'find'},{id:'btnReplace1',c:'b-purple',t:'🔄',h:'replace'}]},{lbl:'Буфер',b:[{id:'btnPaste1',c:'b-orange',t:'📋',h:'paste'},{id:'btnDup1',c:'b-pink',t:'📑',h:'dup'}]},{lbl:'Новая',b:[{id:'btnNewRUB1',c:'b-purple',t:'RUB',h:'newRUB'},{id:'btnNewUSD1',c:'b-indigo',t:'USD',h:'newUSD'}]},{lbl:'Файл',b:[{id:'btnLoad1',c:'b-slate',t:'📂',h:'load'},{id:'btnSave1',c:'b-teal',t:'💾',h:'save'}]},{lbl:'Инструменты',b:[{id:'btnRecalc1',c:'b-teal',t:'🔄',h:'recalc'},{id:'btnClear1',c:'b-red',t:'🗑',h:'clear'}]},{lbl:'Отмена',b:[{id:'btnUndo1',c:'b-slate',t:'↩',h:'undo'}]}];
    grid.innerHTML=s.map(x=>`<div class="tb-cell"><div class="tb-lbl">${x.lbl}</div><div class="tb-btns">${x.b.map(b=>`<button class="btn ${b.c}" id="${b.id}">${b.t} <span class="hk" data-hk="${b.h}">Shift+${hkDisplay(hotkeys[b.h])}</span></button>`).join('')}</div></div>`).join('');
}

// Сайдбар V2
function buildSidebarV2(){
    const sb=Q('#sidebarV2');if(!sb)return;
    const s=[{t:'Строки и колонки',b:[{id:'btnAddRow2',c:'sb-blue',i:'＋',x:'Добавить строку',h:'addRow'},{id:'btnDelRow2',c:'sb-red',i:'－',x:'Удалить строку',h:'delRow'},{id:'btnAddCol2',c:'sb-green',i:'＋',x:'Добавить колонку',h:'addCol'},{id:'btnDelCol2',c:'sb-orange',i:'－',x:'Удалить колонку',h:'delCol'}]},{t:'Поиск',b:[{id:'btnFind2',c:'sb-indigo',i:'🔍',x:'Найти',h:'find'},{id:'btnReplace2',c:'sb-purple',i:'🔄',x:'Заменить',h:'replace'}]},{t:'Действия',b:[{id:'btnPaste2',c:'sb-orange',i:'📋',x:'Вставить',h:'paste'},{id:'btnDup2',c:'sb-pink',i:'📑',x:'Дублировать',h:'dup'},{id:'btnNewRUB2',c:'sb-purple',i:'🆕',x:'Новая RUB',h:'newRUB'},{id:'btnNewUSD2',c:'sb-indigo',i:'🆕',x:'Новая USD',h:'newUSD'}]},{t:'Файл',b:[{id:'btnLoad2',c:'sb-slate',i:'📂',x:'Открыть',h:'load'},{id:'btnSave2',c:'sb-teal',i:'💾',x:'Сохранить',h:'save'},{id:'btnRecalc2',c:'sb-teal',i:'🔄',x:'Пересчёт',h:'recalc'},{id:'btnClear2',c:'sb-red',i:'🗑',x:'Очистить',h:'clear'},{id:'btnUndo2',c:'sb-slate',i:'↩',x:'Отменить',h:'undo'}]}];
...
function delRow(td,idx){if(td.rows.length<=1)return toast('Нельзя удалить строку!',0);td.rows.splice(idx,1);render(td);}
...
function handleGlobalHotkeys(e){
    ...
    if(e.ctrlKey&&!e.shiftKey&&!e.altKey&&e.key.toLowerCase()==='h'){e.preventDefault();replaceInTable();return;}
    ...
    if(e.shiftKey&&!e.ctrlKey&&!e.altKey&&!e.target.closest('input')&&!recordingKey){
        let key=e.key.toUpperCase();if(e.code==='Digit1')key='1';if(e.code==='Digit2')key='2';if(e.code==='Delete')key='DELETE';
        if(key===hotkeys.addRow)clickBtn('btnAddRow2');if(key===hotkeys.delRow)clickBtn('btnDelRow2');
        if(key===hotkeys.addCol)clickBtn('btnAddCol2');if(key===hotkeys.delCol)clickBtn('btnDelCol2');
        if(key===hotkeys.recalc)clickBtn('btnRecalc2');if(key===hotkeys.paste)clickBtn('btnPaste2');
        if(key===hotkeys.load)clickBtn('btnLoad2');if(key===hotkeys.newRUB)clickBtn('btnNewRUB2');
        if(key===hotkeys.newUSD)clickBtn('btnNewUSD2');if(key===hotkeys.dup)clickBtn('btnDup2');
        if(key===hotkeys.clear)clickBtn('btnClear2');
    }
...
function clickBtn(id){const el=document.getElementById(id);if(el)el.click();}
...
function bindAllEvents(){
    document.getElementById('uiV1').onclick=()=>switchUI('v1');
    document.getElementById('uiV2').onclick=()=>switchUI('v2');
    ...
    bv('btnAddRow1',()=>{const t=active();t?addRowEnd(t):toast('Выберите таблицу!',0);});
    bv('btnDelRow1',()=>{const t=active();t?delRowEnd(t):toast('Выберите таблицу!',0);});
    bv('btnAddCol1',()=>{const t=active();t?addColEnd(t):toast('Выберите таблицу!',0);});
    bv('btnDelCol1',()=>{const t=active();t?delColEnd(t):toast('Выберите таблицу!',0);});
    bv('btnRecalc1',()=>{T.forEach(td=>{td.rows.forEach(r=>calc(r,td.cols));updTot(td);});toast('Пересчитано');});
    bv('btnPaste1',paste);bv('btnNewRUB1',()=>addTable('RUB'));bv('btnNewUSD1',()=>addTable('USD'));
    bv('btnSave1',save);bv('btnLoad1',loadViaDialog);
    bv('btnClear1',()=>{if(!T.length)return;if(confirm('Удалить все?')){getArea().innerHTML='';T.length=0;idC=0;actId=null;hist.length=0;upEmpty();toast('Очищено');}});
    bv('btnDup1',()=>dupTable());bv('btnUndo1',undo);bv('btnFind1',findInTable);bv('btnReplace1',replaceInTable);
    
    bv('btnAddRow2',()=>{const t=active();t?addRowEnd(t):toast('Выберите таблицу!',0);});
    bv('btnDelRow2',()=>{const t=active();t?delRowEnd(t):toast('Выберите таблицу!',0);});
    bv('btnAddCol2',()=>{const t=active();t?addColEnd(t):toast('Выберите таблицу!',0);});
    bv('btnDelCol2',()=>{const t=active();t?delColEnd(t):toast('Выберите таблицу!',0);});
    bv('btnRecalc2',()=>{T.forEach(td=>{td.rows.forEach(r=>calc(r,td.cols));updTot(td);});toast('Пересчитано');});
    bv('btnPaste2',paste);bv('btnNewRUB2',()=>addTable('RUB'));bv('btnNewUSD2',()=>addTable('USD'));
    bv('btnSave2',save);bv('btnLoad2',loadViaDialog);
    bv('btnClear2',()=>{if(!T.length)return;if(confirm('Удалить все?')){getArea().innerHTML='';T.length=0;idC=0;actId=null;hist.length=0;upEmpty();toast('Очищено');}});
    bv('btnDup2',()=>dupTable());bv('btnUndo2',undo);bv('btnFind2',findInTable);bv('btnReplace2',replaceInTable);
}

// Подсказки внизу
function buildShortcuts(){
    const bar=Q('#shortcutsBar');if(!bar)return;
    bar.innerHTML='<span><kbd>Shift+'+hkDisplay(hotkeys.addRow)+'</kbd> Строка</span> <span><kbd>Shift+'+hkDisplay(hotkeys.delRow)+'</kbd> Удалить</span> <span><kbd>Shift+'+hkDisplay(hotkeys.addCol)+'</kbd> Колонка</span> <span><kbd>Shift+'+hkDisplay(hotkeys.newRUB)+'</kbd> RUB</span> <span><kbd>Shift+'+hkDisplay(hotkeys.newUSD)+'</kbd> USD</span> <span><kbd>Ctrl+F</kbd> Поиск</span> <span><kbd>Ctrl+T</kbd> Тема</span> <span><kbd>ПКМ</kbd> Меню</span>';
}
