...
function insertRowsIntoTable(td,parsedRows){
    ...
    let startIdx=0;
    if(td.rows.length===1&&isEmptyRow(td.rows[0])){
        startIdx=0;
    }else{
        startIdx=td.rows.length;
    }
    parsedRows.forEach((r,pi)=>{
        ...
        for(let i=0;i<td.cols.length;i++){
            ...
            if(cn.includes('ко-во')||cn.includes('количество')||cn.startsWith('цена')){
                const n=pn(v);if(!isNaN(n))v=n.toFixed(2);
            }
            nr[i]=v;
        }
        if(startIdx===0&&pi===0){
    ...
    render(td);
    toast(`Вставлено строк: ${parsedRows.length}`);
}
...
function matchPriceColumn(headerName){
    const h=String(headerName||'').toLowerCase().replace(/[\s,.\-_]/g,'');
    for(const pattern of PRICE_COLUMN_PATTERNS){
        for(const kw of pattern.keywords){
            if(h.includes(kw.toLowerCase()))return pattern.id;
        }
    }
    return null;
}
...
function getDisplayColumns(headers){
    ...
    for(let i=0;i<headers.length;i++){
        const pid=matchPriceColumn(headers[i]);
        if(pid&&!usedPatterns.has(pid)){
            result.push({index:i,name:headers[i],patternId:pid});
            usedPatterns.add(pid);
        }
    }
    const order=['article','name','mega','distributor','partner','dealer','mrp','rrp'];
    result.sort((a,b)=>order.indexOf(a.patternId)-order.indexOf(b.patternId));
    return result;
}
...
function loadPriceFile(file){
    ...
    reader.onload=function(e){
        try{
            ...
            processPriceWB(wb,file.name);
        }catch(er){
            toast('Ошибка чтения прайс-листа: '+er.message,false);
        }
    };
...
// ==================== ПРОСМОТР ПРАЙСА ====================
function viewPrice(){
    if(!priceData||!priceData.sheets.length){
        toast('Сначала загрузите прайс-лист!',false);
        return;
    }
    const modal=Q('#priceViewModal');
    if(!modal){
        toast('Ошибка интерфейса: окно просмотра не найдено',false);
        return;
    }
    const tabsContainer=Q('#priceViewTabs');
    if(!tabsContainer)return;
    
    tabsContainer.innerHTML=priceData.sheets.map((s,i)=>`<button class="price-view-tab${i===0?' active':''}" onclick="switchPriceViewSheet(${i})">${s.name}</button>`).join('');
    window._currentViewSheet=0;
    renderPriceViewSheet(0);
    modal.classList.add('show');
}

function switchPriceViewSheet(idx){
    window._currentViewSheet=idx;
    document.querySelectorAll('.price-view-tab').forEach((t,i)=>t.classList.toggle('active',i===idx));
    renderPriceViewSheet(idx);
}
...
function renderPriceViewSheet(idx){
    ...
    content.querySelectorAll('tr[data-pv-row]').forEach(tr=>{
        tr.addEventListener('contextmenu',function(e){
            e.preventDefault();
            e.stopPropagation();
            const sIdx=parseInt(this.dataset.pvSheet);
            const rIdx=parseInt(this.dataset.pvRow);
...
function selectPriceViewRow(sheetIdx,rowIdx){
    document.querySelectorAll('#priceViewContent tr').forEach(tr=>tr.classList.remove('selected'));
    const tr=Q(`#priceViewContent tr[data-pv-sheet="${sheetIdx}"][data-pv-row="${rowIdx}"]`);
    if(tr)tr.classList.add('selected');
    priceViewCtxRow={sheetIdx:sheetIdx,rowIdx:rowIdx};
}

function showPriceViewCtx(x,y){
    const cm=Q('#priceViewCtxMenu');
    if(!cm)return;
...
function hidePriceViewCtx(){
    const cm=Q('#priceViewCtxMenu');
    if(cm)cm.style.display='none';
}

function transferFromPriceView(){
    if(!priceViewCtxRow||!priceData)return;
    hidePriceViewCtx();
    
    const sheet=priceData.sheets[priceViewCtxRow.sheetIdx];
    const row=sheet.rows[priceViewCtxRow.rowIdx];
    
    selectedPriceRow={
        sheetName:sheet.name,
        row:row,
        rowIndex:priceViewCtxRow.rowIdx,
        headers:sheet.headers,
        displayCols:sheet.displayCols,
        matchType:'manual',
        articleIdx:sheet.articleIdx,
        nameIdx:sheet.nameIdx,
        matchedCol:-1
    };
    
    Q('#priceViewModal').classList.remove('show');
    setTimeout(()=>showPriceSelect(),200);
}

// ==================== ПОИСК В ПРАЙС-ЛИСТЕ ====================
function searchInPrice(){
    if(!priceData||!priceData.sheets.length){toast('Сначала загрузите прайс-лист!',false);return;}
    Q('#priceSearchModal').classList.add('show');
    setTimeout(()=>Q('#priceSearchInput').focus(),100);
    Q('#priceSearchInput').value='';
}
...
function executeSearch(){
    ...
    priceData.sheets.forEach(sheet=>{
        const articleIdx=sheet.articleIdx;
        const nameIdx=sheet.nameIdx;
...
function showSearchResults(){
    ...
    for(const dc of displayCols){headerHTML+=`<th>${dc.name||'—'}</th>`;}
    ...
    resultsBody.innerHTML=priceSearchResults.map((res,idx)=>{
        ...
        rowHTML+=`<td><span class="badge badge-usd">${res.sheetName}</span></td>`;
        for(const dc of displayCols){const val=res.row[dc.index]||'';rowHTML+=`<td>${val}</td>`;}
...
function showPriceSelect(){
    ...
    infoEl.innerHTML=`
        <div class="info-label">Артикул</div><div class="info-value">${articleVal}</div>
        <div class="info-label" style="margin-top:8px;">Наименование</div><div class="info-value">${nameVal}</div>
        <div class="info-label" style="margin-top:8px;">Лист</div><div class="info-value">${selectedPriceRow.sheetName}</div>`;
    ...
    if(priceButtons.length===0){
        ...
        buttonsGrid.innerHTML=priceButtons.map(pb=>{
            ...
            if(pb.isEmpty){
                return`<button class="price-btn empty-price" disabled><span>${pb.label}</span><span class="price-value">нет цены</span></button>`;
            }else{
                return`<button class="price-btn" onclick="insertPriceFromSearch('${escapedLabel}','${escapedValue}')"><span>${pb.label}</span><span class="price-value">${pb.value}</span></button>`;
            }
        }).join('');
    }
    modal.classList.add('show');
...
function showManualCopy(){
    ...
    const displayCols=selectedPriceRow.displayCols;
    manualHead.innerHTML='<tr>'+displayCols.map(dc=>`<th>${dc.name||'—'}</th>`).join('')+'</tr>';
    manualBody.innerHTML='<tr>'+displayCols.map(dc=>{
        ...
        return`<td class="clickable-cell" onclick="insertManualCell('${dc.patternId}','${escapedVal}')">${displayVal||'—'}</td>`;
    }).join('')+'</tr>';
}
...
function showFallbackResults(sheet){
    ...
    Q('#fallbackHead').innerHTML='<tr>'+displayCols.map(dc=>`<th>${dc.name||'—'}</th>`).join('')+'</tr>';
    window._fallbackSheet=sheet;
    Q('#fallbackBody').innerHTML=sheet.rows.map((row,ri)=>
        `<tr data-fidx="${ri}" onclick="selectFallbackRow(${ri})">`+displayCols.map(dc=>`<td>${row[dc.index]||'—'}</td>`).join('')+`</tr>`
    ).join('');
    modal.classList.add('show');
}
function selectFallbackRow(idx){
    document.querySelectorAll('#fallbackBody tr').forEach(tr=>tr.classList.remove('selected'));
    const tr=Q(`#fallbackBody tr[data-fidx="${idx}"]`);
    if(tr)tr.classList.add('selected');
    window._selectedFallbackIdx=idx;
}
function useFallbackRow(){
    const sheet=window._fallbackSheet;const idx=window._selectedFallbackIdx;
    if(!sheet||idx===undefined||idx<0){toast('Выберите строку в таблице!',false);return;}
    const row=sheet.rows[idx];
    selectedPriceRow={sheetName:sheet.name,row:row,rowIndex:idx,headers:sheet.headers,displayCols:sheet.displayCols,matchType:'manual',articleIdx:sheet.articleIdx,nameIdx:sheet.nameIdx,matchedCol:-1};
    Q('#priceFallbackModal').classList.remove('show');
    showPriceSelect();
}
...
function bindAllEvents(){
    ...
    const safeBind=(id,fn,retries=5)=>{
        ...
        if(!tryBind()&&retries>0){setTimeout(()=>safeBind(id,fn,retries-1),100);}
    };
    ...
    const pvCtx=Q('#priceViewCtxMenu');
    if(pvCtx){
        pvCtx.addEventListener('click',e=>{
