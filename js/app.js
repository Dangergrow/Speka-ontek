...
const PRICE_COLUMN_PATTERNS = [
    ...
    { keywords: ['дистрибутор','дистрибьютор','distributor'], id: 'partner_dist', label: 'Дистрибутор' },
...
function isNumericCol(colName){const cn=String(colName||'').toLowerCase();return cn.includes('ко-во')||cn.includes('количество')||cn.startsWith('цена')||cn.startsWith('стоимость');}
...
function hdrs(cols,cur){
    return cols.map(c=>{
        const t=String(c).trim();
        if(/^цена[,.]?\s*(с\s+ндс)?\s*$/i.test(t))return `Цена, ${cur} с НДС`;
        if(/^стоимость[,.]?\s*(с\s+ндс)?\s*$/i.test(t))return `Стоимость, ${cur} с НДС`;
        return c;
    });
}
...
];sb.innerHTML=s.map(x=>`<div class="sidebar-section"><div class="sidebar-section-title">${x.t}</div>${x.b.map(b=>`<button class="sidebar-btn ${b.c}" id="${b.id}" title="${b.x}"><span class="s-icon">${b.i}</span> ${b.x} ${b.h?`<span class="s-hotkey" data-hk="${b.h}">Shift+${hkDisplay(hotkeys[b.h])}</span>`:''}</button>`).join('')}</div>`).join('');}
...
function buildCardDOM(td){
    ...
    card.addEventListener('contextmenu',e=>{
        ...
        const thEl=e.target.closest('thead th:not(.rn)');
        const trEl=e.target.closest('tbody tr');
        let ri=null,ci=null;
        if(thEl){const ths=[...card.querySelectorAll('thead th:not(.rn)')];ci=ths.indexOf(thEl);if(ci>=td.cols.length)ci=null;}
        if(trEl&&!trEl.classList.contains('tot')){
            ri=+trEl.dataset.ri;
            const tdEl=e.target.closest('td:not(.rn):not(.act-cell)');
            if(tdEl){const tds=[...trEl.querySelectorAll('td:not(.rn):not(.act-cell)')];ci=tds.indexOf(tdEl);if(ci>=td.cols.length)ci=null;}
        }
        ctxD={t:td.id,r:ri,c:ci};setAct(td.id);
        const cm=Q('#ctxMenu');cm.style.display='block';
        cm.style.left=Math.min(e.clientX,window.innerWidth-220)+'px';
        cm.style.top=Math.min(e.clientY,window.innerHeight-400)+'px';
    });
    const hdr=document.createElement('div');
    hdr.className='card-hdr';
    hdr.innerHTML=`<span>📋 ${td.currency} с НДС <span class="badge badge-${td.currency==='USD'?'usd':'rub'}">${td.currency}</span><span class="badge badge-act">Активна</span></span><div class="card-acts"><button class="btn b-outline cur-btn" style="padding:4px 10px;font-size:11px;" title="Конвертировать валюту">${td.currency==='USD'?'💵 USD':'💰 RUB'}</button><button class="btn b-outline dup-btn" style="padding:4px 6px;font-size:11px;" title="Дублировать">📑</button><button class="btn b-red del-btn" style="padding:4px 10px;font-size:11px;" title="Удалить">🗑</button></div>`;
    ...
    hdr.querySelector('.del-btn').onclick=e=>{e.stopPropagation();const ws=workspaces[activeWorkspace];if(ws.length<=1)return toast('Нужна хотя бы одна таблица!',false);card.remove();ws.splice(ws.indexOf(td),1);if(actId===td.id)actId=ws.length?ws[0].id:null;upEmpty();toast('Удалена');};
    const scr=document.createElement('div');scr.className='scroll';
    const tbl=document.createElement('table');tbl.innerHTML='<thead><tr></tr></thead><tbody></tbody>';
    scr.appendChild(tbl);card.appendChild(hdr);card.appendChild(scr);
    td.el=tbl;
    return card;
}
...
function render(td){
    if(!td.el)return;
    const th=td.el.querySelector('thead tr'),tb=td.el.querySelector('tbody');
    th.innerHTML='<th class="rn">№</th>'+hdrs(td.cols,td.currency).map((h,i)=>`<th class="${ccl(td.cols[i])}">${escapeHtml(h)}</th>`).join('')+'<th style="width:44px"></th>';
    tb.innerHTML='';
    td.rows.forEach((r,ri)=>{
        ...
        r.forEach((v,ci)=>{
            const tdEl=document.createElement('td');tdEl.className=ccl(td.cols[ci]);
            const inp=document.createElement('input');
            const cn=(td.cols[ci]||'').toLowerCase();
            if(cn.startsWith('стоимость')){inp.className='total';inp.readOnly=true;}
            inp.value=v??'';
            if(!cn.startsWith('стоимость'))inp.className=(cn.includes('ко-во')||cn.includes('количество'))?'qty':cn.startsWith('цена')?'price':'';
            if(cn.includes('наименование')){inp.style.whiteSpace='pre-wrap';inp.style.wordWrap='break-word';}
            inp.addEventListener('focus',()=>{tb.querySelectorAll('tr').forEach(r=>r.classList.remove('sel'));tr.classList.add('sel');inp.dataset.oldValue=inp.value;});
            if((cn.includes('ко-во')||cn.includes('количество'))||cn.startsWith('цена')){
                inp.addEventListener('input',()=>{td.rows[ri][ci]=inp.value;updCalc(td);});
                inp.addEventListener('blur',()=>{const n=pn(td.rows[ri][ci]);if(!isNaN(n)){td.rows[ri][ci]=n.toFixed(2);inp.value=td.rows[ri][ci];updCalc(td);}});
            }else if(!cn.startsWith('стоимость')){
                inp.addEventListener('input',()=>{td.rows[ri][ci]=inp.value;});
            }
            inp.addEventListener('change',()=>{if(inp.dataset.oldValue!==undefined&&inp.value!==inp.dataset.oldValue){hist.push({a:'editCell',tid:td.id,d:{ri:ri,ci:ci,old:inp.dataset.oldValue,val:inp.value}});}});
            inp.addEventListener('keydown',e=>{
                if(e.key==='Enter'&&e.shiftKey){e.preventDefault();const s=inp.selectionStart,en=inp.selectionEnd,v=inp.value;inp.value=v.substring(0,s)+'\n'+v.substring(en);inp.selectionStart=inp.selectionEnd=s+1;td.rows[ri][ci]=inp.value;}
                else if(e.key==='Tab'&&!e.shiftKey){e.preventDefault();mvFocus(td,ri,ci,1);}
                else if(e.key==='Tab'&&e.shiftKey){e.preventDefault();mvFocus(td,ri,ci,-1);}
                else if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();mvFocus(td,ri,ci,0,1);}
            });
            tdEl.appendChild(inp);tr.appendChild(tdEl);
        });
        const at=document.createElement('td');at.className='act-cell';
        at.innerHTML='<div style="display:flex;gap:3px;justify-content:center;"><button class="rbtn rbtn-add" title="Добавить строку ниже">+</button><button class="rbtn rbtn-del" title="Удалить строку">✕</button></div>';
    ...
    if(td.rows.length>0){
        const totTr=document.createElement('tr');totTr.className='tot';
        const ti=ci(td.cols,'Стоимость');
        totTr.innerHTML='<td class="rn"></td>'+td.cols.map((_,ci)=>{
            if(ci===(ti>=0?ti-1:td.cols.length-2))return'<td style="text-align:right;font-weight:700;padding-right:8px">Итого</td>';
            if(ci===ti)return`<td><input value="${sumT(td).toFixed(2)}" readonly class="total" style="font-weight:700"></td>`;
            return'<td></td>';
        }).join('')+'<td class="act-cell"></td>';
...
function delRow(td,idx){if(td.rows.length<=1)return toast('Нельзя удалить строку!',false);hist.push({a:'delRow',tid:td.id,d:{i:idx,r:[...td.rows[idx]]}});td.rows.splice(idx,1);render(td);}
function addRowEnd(td){td.rows.push(Array(td.cols.length).fill(''));calc(td.rows[td.rows.length-1],td.cols);render(td);}
function delRowEnd(td){if(td.rows.length<=1)return toast('Нельзя удалить строку!',false);hist.push({a:'delRowEnd',tid:td.id,d:{i:td.rows.length-1,r:[...td.rows[td.rows.length-1]]}});td.rows.pop();render(td);}
...
function delCol(td,idx){if(td.cols.length<=2)return toast('Минимум 2 колонки!',false);hist.push({a:'delCol',tid:td.id,d:{i:idx,n:td.cols[idx],c:td.rows.map(r=>r[idx])}});td.cols.splice(idx,1);td.rows.forEach(r=>r.splice(idx,1));render(td);}
...
function delColEnd(td){if(td.cols.length<=2)return toast('Минимум 2 колонки!',false);hist.push({a:'delColEnd',tid:td.id,d:{i:td.cols.length-1,n:td.cols[td.cols.length-1],c:td.rows.map(r=>r[td.cols.length-1])}});td.cols.pop();td.rows.forEach(r=>r.pop());render(td);}
...
function paste(){const td=active();if(!td)return toast('Выберите таблицу!',false);navigator.clipboard.readText().then(text=>{const p=parseCB(text);if(!p.length)return toast('Нет данных',false);insertRowsIntoTable(td,p);}).catch(()=>{const m=prompt('Вставьте:','');if(m){const p=parseCB(m);insertRowsIntoTable(td,p);}});}
...
function insertRowsIntoTable(td,parsedRows){
    ...
    parsedRows.forEach((r,pi)=>{
        ...
        for(let i=0;i<td.cols.length;i++){
            ...
            if(isNumericCol(cn)){const n=pn(v);if(!isNaN(n))v=n.toFixed(2);}
            nr[i]=v;
        }
...
function processPriceWB(wb,fileName){
    const sheets=[];
    wb.SheetNames.forEach(sheetName=>{
        ...
        for(let i=headerRow+1;i<raw.length;i++){
            ...
            if(rowStr==='итого'||rowStr.startsWith('итого'))continue;
            rows.push(row.map(c=>String(c??'').trim()));
        }
        const hasArticle=headers.some(h=>PRICE_COLUMN_PATTERNS.find(p=>p.id==='article').keywords.some(k=>h.toLowerCase().includes(k)));
        const hasName=headers.some(h=>PRICE_COLUMN_PATTERNS.find(p=>p.id==='name').keywords.some(k=>h.toLowerCase().includes(k)));
        const displayCols=getDisplayColumns(headers);
        const articleIdx=headers.findIndex(h=>PRICE_COLUMN_PATTERNS.find(p=>p.id==='article').keywords.some(k=>h.toLowerCase().includes(k)));
        const nameIdx=headers.findIndex(h=>PRICE_COLUMN_PATTERNS.find(p=>p.id==='name').keywords.some(k=>h.toLowerCase().includes(k)));
        sheets.push({name:sheetName,headers:headers,rows:rows,headerRow:headerRow,hasArticle:hasArticle,hasName:hasName,displayCols:displayCols,articleIdx:articleIdx,nameIdx:nameIdx});
    });
    priceData={fileName:fileName,sheets:sheets};
    const totalRows=sheets.reduce((s,sh)=>s+sh.rows.length,0);
    toast(`✅ Прайс загружен: ${sheets.length} листов, ${totalRows} строк`);
}
async function loadPriceViaDialog(){
    if(window.pywebview&&window.pywebview.api){
        try{
            ...
            if(r.success){
                ...
                processPriceWB(XLSX.read(bytes,{type:'array'}),r.name);
            }
        }catch(e){toast('Ошибка загрузки прайса',false);}
    }else{Q('#priceFileInput').click();}
}

// ==================== ПРОСМОТР ПРАЙСА ====================
function viewPrice(){
    if(!priceData||!priceData.sheets.length){toast('Сначала загрузите прайс-лист!',false);return;}
    const modal=Q('#priceViewModal');if(!modal){toast('Ошибка интерфейса',false);return;}
    const tabsContainer=Q('#priceViewTabs');if(!tabsContainer)return;
    tabsContainer.innerHTML=priceData.sheets.map((s,i)=>`<button class="price-view-tab${i===0?' active':''}" onclick="switchPriceViewSheet(${i})">${escapeHtml(s.name)}</button>`).join('');
    window._currentViewSheet=0;
    renderPriceViewSheet(0);
...
function renderPriceViewSheet(idx){
    ...
    const content=Q('#priceViewContent');if(!content)return;
    ...
    sheet.rows.forEach((row,ri)=>{
        ...
        row.forEach((v,ci)=>html+=`<td data-pv-col="${ci}" data-pv-val="${encodeURIComponent(v||'')}" onclick="event.stopPropagation();">${escapeHtml(v||'')}</td>`);
        html+='</tr>';
    });
    html+='</tbody></table></div>';
    content.innerHTML=html;
    content.querySelectorAll('tr[data-pv-row]').forEach(tr=>{
        tr.addEventListener('contextmenu',function(e){
            e.preventDefault();e.stopPropagation();
            const sIdx=parseInt(this.dataset.pvSheet);const rIdx=parseInt(this.dataset.pvRow);
            selectPriceViewRow(sIdx,rIdx);
            const td=e.target.closest('td[data-pv-col]');
            if(td){window._priceViewClickedCell={colIdx:parseInt(td.dataset.pvCol),value:decodeURIComponent(td.dataset.pvVal||'')};}
            else{window._priceViewClickedCell=null;}
            showPriceViewCtx(e.clientX,e.clientY);
        });
    });
}
...
function showPriceViewCtx(x,y){
    ...
    cm.style.top=Math.min(y,window.innerHeight-150)+'px';
    cm.onclick=function(e){
        const item=e.target.closest('.ctx-item');if(!item)return;
        if(item.dataset.action==='transferToTable')transferFromPriceView();
        if(item.dataset.action==='copyCell')copyPriceViewCell();
        hidePriceViewCtx();
    };
}
...
function executeSearch(){
    const query=Q('#priceSearchInput').value.trim();
    if(!query){toast('Введите артикул или наименование',false);return;}
    if(!priceData||!priceData.sheets.length){toast('Прайс не загружен',false);return;}
    const searchType=Q('input[name="searchType"]:checked').value;
    priceSearchResults=[];
    priceData.sheets.forEach(sheet=>{
        const articleIdx=sheet.articleIdx;const nameIdx=sheet.nameIdx;
        sheet.rows.forEach((row,ri)=>{
            let matchType='partial';let matchedCol=-1;
            if(searchType==='article'){
                if(articleIdx>=0){
                    const articleVal=row[articleIdx]||'';
                    if(isExactMatch(articleVal,query)){matchType='exact';matchedCol=articleIdx;}
                    else if(isPartialMatch(articleVal,query)){matchType='partial';matchedCol=articleIdx;}
                    else return;
                }else if(nameIdx>=0){
                    const nameVal=row[nameIdx]||'';
                    if(isPartialMatch(nameVal,query)){matchType='partial';matchedCol=nameIdx;}else return;
                }else return;
            }else{
                if(nameIdx>=0){
                    const nameVal=row[nameIdx]||'';
                    if(isExactMatch(nameVal,query)){matchType='exact';matchedCol=nameIdx;}
                    else if(isPartialMatch(nameVal,query)){matchType='partial';matchedCol=nameIdx;}
                    else return;
                }else if(articleIdx>=0){
                    const articleVal=row[articleIdx]||'';
                    if(isPartialMatch(articleVal,query)){matchType='partial';matchedCol=articleIdx;}else return;
                }else return;
            }
            priceSearchResults.push({sheetName:sheet.name,row:row,rowIndex:ri,headers:sheet.headers,displayCols:sheet.displayCols,matchType:matchType,articleIdx:articleIdx,nameIdx:nameIdx,matchedCol:matchedCol});
        });
    });
    priceSearchResults.sort((a,b)=>{
...
function showSearchResults(){
    const resultsModal=Q('#priceResultsModal');
    const resultsHead=Q('#priceResultsHead');
    const resultsBody=Q('#priceResultsBody');
    const resultsEmpty=Q('#priceResultsEmpty');
    const resultsTable=Q('#priceResultsTable');
    const resultsCount=Q('#resultsCount');
    const resultsInfo=Q('#resultsInfo');
    resultsCount.textContent=priceSearchResults.length;
    if(!priceSearchResults.length){
        resultsHead.innerHTML='';resultsBody.innerHTML='';
        resultsEmpty.style.display='block';resultsTable.style.display='none';
        resultsInfo.textContent='Ничего не найдено. Попробуйте изменить запрос или тип поиска.';
        resultsModal.classList.add('show');return;
    }
    resultsEmpty.style.display='none';resultsTable.style.display='table';
    const firstResult=priceSearchResults[0];
    const displayCols=firstResult.displayCols;
    let headerHTML='<tr><th>Лист</th>';
    for(const dc of displayCols){headerHTML+=`<th>${escapeHtml(dc.name||'—')}</th>`;}
    headerHTML+='</tr>';resultsHead.innerHTML=headerHTML;
    resultsBody.innerHTML=priceSearchResults.map((res,idx)=>{
        let rowHTML=`<tr class="${res.matchType==='exact'?'exact-match':''}" data-idx="${idx}" onclick="selectSearchResult(${idx})">`;
        rowHTML+=`<td><span class="badge badge-usd">${escapeHtml(res.sheetName)}</span></td>`;
        for(const dc of displayCols){const val=res.row[dc.index]||'';rowHTML+=`<td>${escapeHtml(val)}</td>`;}
        rowHTML+='</tr>';return rowHTML;
    }).join('');
    const exactCount=priceSearchResults.filter(r=>r.matchType==='exact').length;
    resultsInfo.textContent=`Точных совпадений: ${exactCount}, частичных: ${priceSearchResults.length-exactCount}`;
    resultsModal.classList.add('show');
}

function selectSearchResult(idx){
    document.querySelectorAll('#priceResultsBody tr').forEach(tr=>tr.classList.remove('selected'));
    const tr=Q(`#priceResultsBody tr[data-idx="${idx}"]`);
    if(tr)tr.classList.add('selected');
    selectedPriceRow=priceSearchResults[idx];
    setTimeout(()=>{Q('#priceResultsModal').classList.remove('show');showPriceSelect();},200);
}

function showPriceSelect(){
    if(!selectedPriceRow){toast('Строка не выбрана',false);return;}
    const modal=Q('#priceSelectModal');
    const infoEl=Q('#selectedItemInfo');
    const buttonsGrid=Q('#priceButtonsGrid');
    const manualArea=Q('#manualCopyArea');
    manualArea.style.display='none';
    if(buttonsGrid)buttonsGrid.style.display='grid';
    const articleVal=selectedPriceRow.articleIdx>=0?selectedPriceRow.row[selectedPriceRow.articleIdx]:'—';
    const nameVal=selectedPriceRow.nameIdx>=0?selectedPriceRow.row[selectedPriceRow.nameIdx]:'—';
    infoEl.innerHTML=`<div class="info-label">Артикул</div><div class="info-value">${escapeHtml(articleVal)}</div><div class="info-label" style="margin-top:8px;">Наименование</div><div class="info-value">${escapeHtml(nameVal)}</div><div class="info-label" style="margin-top:8px;">Лист</div><div class="info-value">${escapeHtml(selectedPriceRow.sheetName)}</div>`;
    const displayCols=selectedPriceRow.displayCols;
    const priceButtons=[];
    for(const dc of displayCols){
        if(dc.patternId==='article'||dc.patternId==='name')continue;
        const val=selectedPriceRow.row[dc.index]||'';
        const isEmpty=!val||String(val).trim()==='';
        const numVal=pn(val);
        const displayVal=(!isEmpty&&!isNaN(numVal))?numVal.toFixed(2):val;
        priceButtons.push({label:dc.name,value:displayVal,patternId:dc.patternId,isEmpty:isEmpty,colIndex:dc.index});
    }
    if(priceButtons.length===0){
        buttonsGrid.innerHTML='<div style="grid-column:1/-1;text-align:center;color:var(--text2);padding:20px;">Нет колонок с ценами.<br>Используйте «Скопировать вручную».</div>';
    }else{
        buttonsGrid.innerHTML=priceButtons.map(pb=>{
            const escapedValue=String(pb.value).replace(/'/g,"\\'").replace(/"/g,'&quot;');
            const escapedLabel=pb.label.replace(/'/g,"\\'");
            if(pb.isEmpty){
                return`<button class="price-btn empty-price" disabled><span>${escapeHtml(pb.label)}</span><span class="price-value">нет цены</span></button>`;
            }else{
                return`<button class="price-btn" onclick="insertPriceFromSearch('${escapedLabel}','${escapedValue}')"><span>${escapeHtml(pb.label)}</span><span class="price-value">${escapeHtml(pb.value)}</span></button>`;
            }
        }).join('');
    }
    modal.classList.add('show');
...
function insertPriceFromSearch(priceType,priceValue){
    const td=active();
    if(!td){toast('Выберите активную таблицу!',false);return;}
    if(!selectedPriceRow){toast('Строка не выбрана',false);return;}
    const articleVal=selectedPriceRow.articleIdx>=0?(selectedPriceRow.row[selectedPriceRow.articleIdx]||''):'';
    const nameVal=selectedPriceRow.nameIdx>=0?(selectedPriceRow.row[selectedPriceRow.nameIdx]||''):'';
    const articleCol=ci(td.cols,'Артикул');
    const nameCol=ci(td.cols,'Наименование');
    const priceCol=ci(td.cols,'Цена');
    const numPrice=pn(priceValue);
    const finalPrice=!isNaN(numPrice)?numPrice.toFixed(2):String(priceValue);
    const newRow=Array(td.cols.length).fill('');
    if(articleCol>=0)newRow[articleCol]=articleVal;
    if(nameCol>=0)newRow[nameCol]=nameVal;
    if(priceCol>=0)newRow[priceCol]=finalPrice;
    insertRowIntoTable(td,newRow);
    Q('#priceSelectModal').classList.remove('show');
    selectedPriceRow=null;
    toast(`✅ Добавлено: ${articleVal||nameVal} (${priceType})`);
}

function showManualCopy(){
    if(!selectedPriceRow)return;
    const manualArea=Q('#manualCopyArea');
    const manualHead=Q('#manualCopyHead');
    const manualBody=Q('#manualCopyBody');
    const priceButtonsGrid=Q('#priceButtonsGrid');
    if(priceButtonsGrid)priceButtonsGrid.style.display='none';
    manualArea.style.display='block';
    Q('#btnCancelManual').style.display='block';
    const displayCols=selectedPriceRow.displayCols;
    manualHead.innerHTML='<tr>'+displayCols.map(dc=>`<th>${escapeHtml(dc.name||'—')}</th>`).join('')+'</tr>';
    manualBody.innerHTML='<tr>'+displayCols.map(dc=>{
        const val=selectedPriceRow.row[dc.index]||'';
        const numVal=pn(val);
        const displayVal=(!isNaN(numVal))?numVal.toFixed(2):val;
        const escapedVal=String(displayVal).replace(/'/g,"\\'").replace(/"/g,'&quot;');
        return`<td class="clickable-cell" onclick="insertManualCell('${dc.patternId}','${escapedVal}')">${escapeHtml(displayVal||'—')}</td>`;
    }).join('')+'</tr>';
}

function insertManualCell(patternId,value){
    const td=active();
    if(!td){toast('Выберите активную таблицу!',false);return;}
    if(!selectedPriceRow){toast('Строка не выбрана',false);return;}
    const articleVal=selectedPriceRow.articleIdx>=0?(selectedPriceRow.row[selectedPriceRow.articleIdx]||''):'';
    ...
    const nameCol=ci(td.cols,'Наименование');
    const priceCol=ci(td.cols,'Цена');
    const numPrice=pn(value);
    const finalValue=!isNaN(numPrice)?numPrice.toFixed(2):String(value);
    const newRow=Array(td.cols.length).fill('');
    if(articleCol>=0)newRow[articleCol]=articleVal;
    if(nameCol>=0)newRow[nameCol]=nameVal;
    if(patternId==='article'&&articleCol>=0)newRow[articleCol]=finalValue;
    else if(patternId==='name'&&nameCol>=0)newRow[nameCol]=finalValue;
    else if(priceCol>=0)newRow[priceCol]=finalValue;
    insertRowIntoTable(td,newRow);
    Q('#priceSelectModal').classList.remove('show');
    selectedPriceRow=null;
    toast(`✅ Добавлено: ${articleVal||nameVal}`);
}

function insertRowIntoTable(td,newRow){
    if(td.rows.length===1&&isEmptyRow(td.rows[0])){td.rows[0]=newRow;}
    else{td.rows.push(newRow);}
    calc(newRow,td.cols);
    render(td);
}

// ==================== FALLBACK ====================
function showFallbackResults(sheet){
    const modal=Q('#priceFallbackModal');
    Q('#fallbackInfo').textContent=`Лист «${sheet.name}» — не найдены колонки «Артикул» и «Наименование». Выберите строку:`;
    const displayCols=sheet.displayCols;
    Q('#fallbackHead').innerHTML='<tr>'+displayCols.map(dc=>`<th>${escapeHtml(dc.name||'—')}</th>`).join('')+'</tr>';
    window._fallbackSheet=sheet;
    Q('#fallbackBody').innerHTML=sheet.rows.map((row,ri)=>
        `<tr data-fidx="${ri}" onclick="selectFallbackRow(${ri})">`+displayCols.map(dc=>`<td>${escapeHtml(row[dc.index]||'—')}</td>`).join('')+`</tr>`
    ).join('');
    modal.classList.add('show');
}
...
function makeDraggable(dialogId,handleId){
    ...
    handle.addEventListener('mousedown',function(e){
        ...
        dialog.style.left=initialLeft+'px';dialog.style.top=initialTop+'px';
    });
...
function save(){
    ...
    if(!ws.length)return toast('Нет таблиц!',false);
    const wb=new ExcelJS.Workbook();
    const wsheet=wb.addWorksheet('Заказы');
    let cr=1;
    const mc=Math.max(...ws.map(t=>t.cols.length),5);
    wsheet.columns=Array(mc).fill({width:24});
    ws.forEach(td=>{
        const ni=ci(td.cols,'Наименование');if(ni>=0)wsheet.getColumn(ni+1).width=88;
        const qi=getQi(td.cols);if(qi>=0)wsheet.getColumn(qi+1).width=18;
        const pi=ci(td.cols,'Цена');if(pi>=0)wsheet.getColumn(pi+1).width=22;
        const toti=ci(td.cols,'Стоимость');if(toti>=0)wsheet.getColumn(toti+1).width=22;
    });
    ws.forEach(td=>{
        if(!td.rows.length)return;
        const tc=td.cols.length,qi=getQi(td.cols),pi=ci(td.cols,'Цена'),toti=ci(td.cols,'Стоимость'),ni=ci(td.cols,'Наименование');
        const hr=wsheet.getRow(cr);
        hdrs(td.cols,td.currency).forEach((h,ci)=>{
            ...
            c.border={top:{style:'thin',color:{argb:'FF475569'}},bottom:{style:'thin',color:{argb:'FF475569'}},left:{style:'thin',color:{argb:'FF475569'}},right:{style:'thin',color:{argb:'FF475569'}}};
        });
        hr.height=28;cr++;
        const fdr=cr;
        for(let r=0;r<td.rows.length;r++){
            const row=wsheet.getRow(cr),bg=r%2===0?'FFEFF6FF':'FFDBEAFE';
            for(let c=0;c<tc;c++){
                const cell=row.getCell(c+1);
                if(c===toti)cell.value={formula:`${cA(cr,qi+1)}*${cA(cr,pi+1)}`,result:pn(td.rows[r][c])||0};
                else if(c===qi||c===pi){const v=pn(td.rows[r][c]);cell.value=isNaN(v)?td.rows[r][c]:v;}
                else cell.value=td.rows[r][c]??'';
                if(c===toti||c===qi||c===pi)cell.numFmt='#,##0.00';
                ...
                if(c===qi||c===pi||c===toti)cell.alignment.horizontal='right';
            }
            cr++;
        }
        const ldr=cr-1,totr=wsheet.getRow(cr);
        for(let c=0;c<tc;c++){
            const cell=totr.getCell(c+1);
            if(c===(toti>=0?toti-1:tc-2))cell.value='Итого';
            else if(c===toti)cell.value={formula:`SUM(${cA(fdr,toti+1)}:${cA(ldr,toti+1)})`,result:sumT(td)};
            if(c===toti)cell.numFmt='#,##0.00';
            cell.font={bold:true,size:12,name:'Calibri'};
            cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFE2E8F0'}};
            cell.alignment={horizontal:'right',vertical:'middle',wrapText:true};
            cell.border={top:{style:'medium',color:{argb:'FF64748B'}},bottom:{style:'medium',color:{argb:'FF64748B'}},left:{style:'thin',color:{argb:'FFBFDBFE'}},right:{style:'thin',color:{argb:'FFBFDBFE'}}};
        }
        totr.height=28;cr++;
    });
    wb.xlsx.writeBuffer().then(buf=>{
        ...
        if(window.pywebview&&window.pywebview.api){
            ...
            window.pywebview.api.save_file(b64,fn).then(r=>{const j=JSON.parse(r);if(j.success)toast('✅ Сохранено!');else toast('Отменено',false);});
        }else{
            const blob=new Blob([buf],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
            const url=URL.createObjectURL(blob);
            const a=document.createElement('a');a.href=url;a.download=fn;document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);toast('✅ Сохранено!');
        }
    }).catch(e=>{toast('Ошибка сохранения',false);});
}

function load(file){const r=new FileReader();r.onload=e=>{try{const wb=XLSX.read(new Uint8Array(e.target.result),{type:'array'});processWB(wb);}catch(er){toast('Ошибка',false);if(!workspaces[activeWorkspace].length)addTable('USD');}};r.readAsArrayBuffer(file);}
...
function processWB(wb){
    ...
    wb.SheetNames.forEach(n=>{
        ...
        for(let i=0;i<raw.length;i++){
            const row=raw[i];
            const emp=!row||row.every(c=>String(c??'').trim()==='');
            ...
            // Заголовок таблицы
            if(row.some(c=>['артикул','наименование'].some(k=>String(c||'').toLowerCase().includes(k)))){
                if(cur&&cur.rows.length)fin(cur);
                skipCols=0;
                let hs=row.map(c=>String(c||'').trim());
                // Пропускаем ВСЕ ведущие "№"
                while(hs.length&&/^№+$/i.test(hs[0].trim())){hs=hs.slice(1);skipCols++;}
                const hd=hs.map(c=>c.replace(/,?\s*(USD|RUB)\s*с НДС/i,''));
                const clean=hd.filter(h=>h);
                const det=row.join(' ').includes('USD')?'USD':'RUB';
                cur={cols:clean.length?clean:[...DC],rows:[],currency:det};
                continue;
            }
            
            if(!cur)cur={cols:[...DC],rows:[],currency:'RUB'};
            
            // Сдвигаем если были колонки "№"
            let dr=skipCols?row.slice(skipCols):row.slice();
            const nr=[];
            for(let ci2=0;ci2<cur.cols.length;ci2++){
                let v=String(dr[ci2]??'').trim();
                const cn=(cur.cols[ci2]||'').toLowerCase();
                // pn() ТОЛЬКО для числовых колонок
                if(isNumericCol(cn)){
                    const num=pn(v);
        ...
        function fin(c){
            ...
            idC++;
            ws.push({id:idC,cols:c.cols,rows:c.rows,currency:c.currency,el:null,card:null});
        }
    });
...
function renderHotkeyList(){const container=Q('#hotkeyList');if(!container)return;container.innerHTML=Object.keys(defaultHotkeys).map(k=>{const key=hotkeys[k]||'';return`<div class="hotkey-row"><span class="hk-label">${escapeHtml(keyLabels[k]||k)}</span><span class="hk-current${!key?' conflict':''}" data-hk="${k}">${key?'Shift+'+hkDisplay(key):'—'}</span></div>`;}).join('');container.querySelectorAll('.hk-current').forEach(el=>{el.onclick=()=>startRecording(el);});}
...
Q('#ctxMenu').addEventListener('click',e=>{
    const item=e.target.closest('.ctx-item');if(!item)return;
    const a=item.dataset.action;hideCtx();
    const td=active();
    if(!td&&!['paste','dupTable'].includes(a))return;
    if(td)setAct(td.id);
    switch(a){
        case'addRowAbove':if(td&&ctxD.r!==null)insRowAbove(td,ctxD.r);break;
        case'addRowBelow':if(td&&ctxD.r!==null)insRowBelow(td,ctxD.r);break;
        case'delRow':if(td&&ctxD.r!==null)delRow(td,ctxD.r);break;
        case'renameCol':if(td&&ctxD.c!==null)renameCol(td,ctxD.c);break;
        case'addColBefore':if(td&&ctxD.c!==null)insCol(td,ctxD.c);break;
        case'addColAfter':if(td&&ctxD.c!==null)insCol(td,ctxD.c+1);break;
        case'delCol':if(td&&ctxD.c!==null)delCol(td,ctxD.c);break;
        case'paste':paste();break;
        case'dupTable':if(td)dupTable(td);else dupTable();break;
        case'delTable':if(td&&workspaces[activeWorkspace].length>1){td.card.remove();const ws=workspaces[activeWorkspace];ws.splice(ws.indexOf(td),1);if(actId===td.id)actId=ws.length?ws[0].id:null;upEmpty();}break;
    }
});
...
function handleGlobalHotkeys(e){
    ...
    if(e.shiftKey&&!e.ctrlKey&&!e.altKey&&!e.target.closest('input')&&!recordingKey){
        ...
        const actions={
            addRow:()=>{e.preventDefault();const t=active();t?addRowEnd(t):toast('Выберите таблицу!',false);},
            delRow:()=>{e.preventDefault();const t=active();t?delRowEnd(t):toast('Выберите таблицу!',false);},
            addCol:()=>{e.preventDefault();const t=active();t?addColEnd(t):toast('Выберите таблицу!',false);},
            delCol:()=>{e.preventDefault();const t=active();t?delColEnd(t):toast('Выберите таблицу!',false);},
            recalc:()=>{e.preventDefault();workspaces[activeWorkspace].forEach(td=>{td.rows.forEach(r=>calc(r,td.cols));updTot(td);});toast('Пересчитано');},
            paste:()=>{e.preventDefault();paste();},
            load:()=>{e.preventDefault();loadViaDialog();},
            newRUB:()=>{e.preventDefault();addTable('RUB');},
            newUSD:()=>{e.preventDefault();addTable('USD');},
            dup:()=>{e.preventDefault();dupTable();},
            clear:()=>{e.preventDefault();const a=getArea();if(!workspaces[activeWorkspace].length)return;if(confirm('Удалить все?')){a.innerHTML='';workspaces[activeWorkspace].length=0;idC=0;actId=null;upEmpty();toast('Очищено');}},
            undo:()=>{e.preventDefault();undo();},
            save:()=>{e.preventDefault();save();},
            convert:()=>{e.preventDefault();const t=active();if(t)convertCurrency(t);else toast('Выберите таблицу!',false);}
        };
...
function gPaste(e){const td=active();if(!td)return;if(e)e.preventDefault();navigator.clipboard.readText().then(text=>{const p=parseCB(text);if(!p.length)return toast('Нет данных',false);insertRowsIntoTable(td,p);}).catch(()=>{});}
...
async function checkUpdate(){
    ...
    try{
        const r=await fetch('https://raw.githubusercontent.com/Dangergrow/Speka-ontek/main/version.json',{cache:'no-cache'});
        const d=await r.json();
        if(d.version>'4.8.0'){
            if(confirm(`🆕 Версия ${d.version}!\n\n${d.notes||''}\n\nОбновить?`)){
                if(window.pywebview&&window.pywebview.api){
                    const result=JSON.parse(await window.pywebview.api.apply_update());
                    if(result.success){toast('✅ Обновлено!');setTimeout(()=>location.reload(),1500);}
                    else toast('Ошибка',false);
                }
            }
        }else toast('✅ Последняя версия');
    }catch(e){toast('Ошибка проверки',false);}
}
...
function bindAllEvents(){
    ...
    safeBind('btnAddRow',()=>{const t=active();t?addRowEnd(t):toast('Выберите таблицу!',false);});
    safeBind('btnDelRow',()=>{const t=active();t?delRowEnd(t):toast('Выберите таблицу!',false);});
    safeBind('btnAddCol',()=>{const t=active();t?addColEnd(t):toast('Выберите таблицу!',false);});
    safeBind('btnDelCol',()=>{const t=active();t?delColEnd(t):toast('Выберите таблицу!',false);});
    safeBind('btnConvert',()=>{const t=active();if(t)convertCurrency(t);else toast('Выберите таблицу!',false);});
    ...
    safeBind('btnFind',findInTable);
    safeBind('btnLoadPrice',loadPriceViaDialog);
    safeBind('btnViewPrice',viewPrice);
    safeBind('btnSearchPrice',searchInPrice);
